...
export default function EditInventoryPage() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          fetchProject(item.projectId)
        }
      } else {
        console.error('Failed to fetch inventory item:', response.status, response.statusText)
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const minSquareFeet = parseFloat(formData.minSquareFeet)
    const pricePerSquareFoot = parseFloat(formData.pricePerSquareFoot)

    if (isNaN(totalArea) || totalArea <= 0) {
      alert('Total area must be a valid positive number')
      setSaving(false)
      return
    }
    if (isNaN(minSquareFeet) || minSquareFeet <= 0) {
      alert('Minimum square feet must be a valid positive number')
      setSaving(false)
      return
    }
    if (isNaN(pricePerSquareFoot) || pricePerSquareFoot <= 0) {
      alert('Price per square foot must be a valid positive number')
      setSaving(false)
      return
    }
    ...
    try {
      ...
      // Prepare update data - convert numeric fields properly
      const updateData = {
